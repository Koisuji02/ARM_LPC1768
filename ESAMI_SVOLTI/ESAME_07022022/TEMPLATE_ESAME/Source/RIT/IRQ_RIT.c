...
int disattivato =  0;
int flagInt0 = 0; // per soegnere il singolo led nel timer e non tutti
int flagKey1 = 0;
int flagKey2 = 0;
int totale = 0;
...
void RIT_IRQHandler(void) {
		...
		if(disattivato == 0){
		// GESTIONE BOTTONE INT0 -> insoddisfatto (LED6) -> 2
			...
			// GESTIONE BOTTONE KEY1 -> soddisfatto (LED4) -> 0
			...
			// GESTIONE BOTTONE KEY2 -> neutro LED5 -> 1
		...
		if ((LPC_GPIO1->FIOPIN & (1 << 29)) == 0) { // JOYSTICK UP
				...
				// AZIONE RIGHT
				totale = stat_satisfaction(S,N,I,&PS,&PN,&PI);
				LED_Out(totale);
				disattivato = 1;
				enable_timer(1);
