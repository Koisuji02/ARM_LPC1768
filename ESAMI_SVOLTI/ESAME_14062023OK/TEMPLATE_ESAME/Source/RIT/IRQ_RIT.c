...
int diff_max_min(int buffer[], unsigned int pos);
...
void RIT_IRQHandler(void) {
		...
		// GESTIONE BOTTONE KEY1 (entro solo dopo visualizzazione result, quindi 1)
		if((LPC_PINCON->PINSEL4 & (1 << 22)) == 0 && disattivato == 1){ // bottone premuto
			//if(disattivato == 0){
				debounce1++;
				// INT0 = 10; KEY1 = 11; KEY2 = 12
				if((LPC_GPIO2->FIOPIN & (1<<11)) == 0){   //bottone premuto ma in modalità interrupt con debounce attivo
					...
					switch(debounce1){
						case 1: // dopo il time_interval con cui init_RIT (1 colpo, no debounce)
							...
							// AZIONE KEY1
							LED_Out(0); // spengo led result
							svuotaVet(vet, index);
							disattivato = 0;
							index = 0;
							enable_timer(3); // riparte
							enable_timer(0);
