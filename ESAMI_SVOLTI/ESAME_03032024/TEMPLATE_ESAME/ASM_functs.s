...
compare							PROC
								...
								MOV r4, #0	; r4 = counter differenze coerenti
								MOV r5, #0	; posizione su cui itero su vet
								;MOV r6, #0	; posizione su cui itero su vet2
								
loop_compare_1					CMP r5, r2
								BGE end_compare_e
								LDRB r7, [r0]	; r7 = vet[i]
								CMP r7, #1
								BNE loop_compare_2
...
loop_compare_2					CMP r7, #2
								ADDNE r0, r0, #1		; vet++
								ADDNE r5, r5, #1		; pos_vet1++
								BNE loop_compare_1
								BEQ vet2_compare

vet2_compare					LDRB r8, [r1]		; r8 = vet2[i]
								ADD r1, r1, #1		; vet2++
								ADD r0, r0, #1		; vet++
								ADD r5, r5, #1		; pos_vet1++
								CMP r7, r8
								BNE loop_compare_1	; se diversi salta
								ADD r4, r4, #1
								B loop_compare_1
								
								
end_compare_e					MOV r0, r4		; torno totale counter
								POP{r4-r8, r10-r11, pc}
								ENDP
