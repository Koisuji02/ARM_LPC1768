...
void svuotaVet(uint32_t* vet, unsigned int index);
				...
				if((LPC_GPIO2->FIOPIN & (1<<11)) == 0){   //bottone premuto ma in modalità interrupt con debounce attivo
					...
					switch(debounce1){
						case 1: // dopo il time_interval con cui init_RIT (1 colpo, no debounce)
							...
							// AZIONE KEY1
							enable_timer(0); // attivo quando premo KEY1
							enable_timer(1); // a valle fare i led alternati
							enable_timer(2); // ogni 5s metto var in vett[index]
				...
				if((LPC_GPIO2->FIOPIN & (1<<12)) == 0){   //bottone premuto ma in modalità interrupt con debounce attivo
					...
					switch(debounce2){
						case 1: // dopo il time_interval con cui init_RIT (1 colpo, no debounce)
							...
							// AZIONE KEY2
							disable_timer(0);
							disable_timer(1); // spengo anche quello a valle, perchè ora sono a monte (finito)
							reset_timer(0);
							reset_timer(1);
							//
							break;
