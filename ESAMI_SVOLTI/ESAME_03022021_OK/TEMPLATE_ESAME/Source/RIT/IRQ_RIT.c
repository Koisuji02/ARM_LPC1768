...
void svuotaVet(int* vet, unsigned int index);
...
void customLedOut(unsigned int value) {
  int i;
  for (i = 1; i < 8; i++) { // escludo LED4 -> 0, così richiesto
    if (value & (1<<i)) {
      LED_On (i);
    } else {
      LED_Off(i);
...
void RIT_IRQHandler(void) {
		...
		if ((LPC_GPIO1->FIOPIN & (1 << 29)) == 0) { // JOYSTICK UP
				...
				// AZIONE DOWN
			if(disattivato == 0) var1 -= 8;
		} else if((LPC_GPIO1->FIOPIN & (1 << 25)) == 0){ // SELECT (R3,L3)
			// AZIONE SELECT
			disattivato = 1; // READY -> ricorda di riazzerare
			vett[index] = var1;
			index++;
			if(index == N){ // vett pieno
				totale = count_negative_and_odd(vett, index);
				svuotaVet(vett, index);
				if(totale == 0){
					LED_On(0); // LED4
					enable_timer(0); // dopo 1.3s spegne
				} else{
					LED_Off(0);
					customLedOut(totale); // senza LED4 (copiata da LED_out, moddata)
				}
			}
			var1 = 0; // riazzero e riparto
			disattivato = 0;
		}
