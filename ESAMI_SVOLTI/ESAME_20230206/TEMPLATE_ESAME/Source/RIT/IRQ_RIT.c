...
void RIT_IRQHandler(void) {
				...
				if((LPC_GPIO2->FIOPIN & (1<<10)) == 0){   //bottone premuto ma in modalità interrupt con debounce attivo
					...
					switch(debounce0){
						case 1: // dopo il time_interval con cui init_RIT (1 colpo, no debounce)
							...
							// AZIONE INT0
						if(pressione2 == 0){
								result = differenza_media_positivi_negativi(vet, N, &over);
								if(over == 0){ // no overflow
									if(result >= 0) LED_Out(0xFF); // tutti accesi
									else{
										LED_Out(0); // pulisco
										LED_On(7);	// LED4 acceso
									}
								} else{ // overflow
									enable_timer(1);
								}
								pressione2 = 1;
							} else{ // 2^ pressione di INT0
								svuotaVet(vet, N);
								LED_Out(0);
								index = 0;
								pressione2 = 0;
		...
		if ((LPC_GPIO1->FIOPIN & (1 << 29)) == 0) { // JOYSTICK UP
				...
				// AZIONE LEFT
		} else if ((LPC_GPIO1->FIOPIN & (1 << 26)) == 0 && pressione2 == 0) { // JOYSTICK DOWN
				// AZIONE DOWN
			disable_timer(2);
			vet[index] = LPC_TIM2->TC;
			index++;
			if(index == N){
				index = 0;
				counterFull++;
				if(counterFull >= 4){
					...
					LED_Out(counterFull); // basta LED_OUT perchè parto dai primi LED
				}
			}
			enable_timer(2);
		}
