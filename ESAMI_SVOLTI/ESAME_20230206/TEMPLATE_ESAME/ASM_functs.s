...
end_sort						MOV r0, r1                      ; r0 = metto val e torno
								...
								EXPORT svuotaVet ; su 8 bit -> 32 bit
...
differenza_media_positivi_negativi PROC
								...
								; r0 = vet (32 bit)
								; r1 = N = indexVetPieno
								; r2 = indirizzo di over (32 bit indirizzo -> 8 bit over)
								
								MOV r4, #0		; valore totale positivi
								MOV r5, #0		; valore totale negativi
								MOV r6, #0		; # positivi
								MOV r7, #0		; # negativi
								MOV r8, #0		; confronto con r1 (N)
								MOV r10, #0xFF		; 0xFF per overflow
								MOV r11, #0		; result
								
loop_differenza					CMP r8, r1
								BGE calcolo_differenza_medie
								LDR r8, [r0]	; vet[i]
								ADD r0, r0, #4	; vet++
								ADD r8, r8, #1	; pos++
								CMP r8, #0		; vedo se positivo (GE)
								BGE sommo_positivi
								BLT sommo_negativi
								
sommo_positivi					ADDS r4, r4, r8
								STRBVS r10, [r2]
								ADD r6, r6, #1
								B loop_differenza
								
sommo_negativi					ADDS r5, r5, r8
								STRBVS r10, [r2]
								ADD r7, r7, #1
								B loop_differenza
								
calcolo_differenza_medie		CMP r6, #0		; se non ho positivi, skippo la differenza
								BEQ media_negativi
								CMP r7, #0 		; se non ho negativi, skippo
								BEQ media_positivi
								; altrimenti fai vera differenza
								SDIV r4, r4, r6	; media positivi
								SDIV r5, r5, r7	; media negativi
								SUB r11, r4, r5	; result
								B end_differenza
								
media_positivi					SDIV r11, r4, r6	; media positivi
								B end_differenza
media_negativi					SDIV r11, r5, r7	; media negativi
								B end_differenza
								
								
end_differenza					MOV r0, r11
								POP{r4-r8, r10-r11, pc}
								ENDP
