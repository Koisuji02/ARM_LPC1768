...
diff_max_min					PROC
								...
								; r0 = vet
								; r1 = index
								MOV r4, #0		; max (riuso per differenza alla fine da tornare)
								MOV r5, #0		; min
								MOV r6, #0		; posizione confronto vet
								LDR r7, [r0]	; r7 = min
								LDR r8, [r0]	; r8 = max
								ADD r0, r0, #4	; vet++
								SUB r1, r1, #1	; visto che 1 l'ho già preso fuori
								
loop_max_min					CMP r6, r1		; fine ciclo
								BGE end_max_min
								LDR r9, [r0]	; vet[i]
								CMP r9, r7		; se vet[i] < min, min = vet[i]
								MOVLT r7, r9
								CMP r9, r8		; se vet[i] > max , max = vet[i]
								MOVGT r8, r9
								ADD r0, r0, #4	; vet++
								ADD r6, r6, #1	; pos++
								B loop_max_min
								
end_max_min						SUB r0, r8, r7		; torno diff
								POP{r4-r8, r10-r11, pc}
								ENDP
