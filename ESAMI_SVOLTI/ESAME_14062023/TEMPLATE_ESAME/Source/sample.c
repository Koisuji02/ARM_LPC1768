...
int main (void) {
  ...
  BUTTON_init();												/* BUTTON Initialization              */
	...
	init_timer(0,0,0,3,0x3F52F0); // si interrupt (SRI = 3 = 011); 166ms = 0.166s
	
	//init_timer(1,0,0,3,0x2625A0); // decimi di secondo = 0.1s
	
	//init_timer(2,0,0,3,0x2FAF080); // 0.5Hz = 2s
	
	init_timer(3,0,0,2,0x1312D0); // 50ms = 0.05s -> SRI = 010 (no interrupt)
	
	enable_timer(3);
	enable_timer(0); // per catturare il TC di TIM3
