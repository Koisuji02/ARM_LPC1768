...
concat_sum						PROC
								...
								; r0 = vet
								; r1 = index
								; r2 = &alarm (32 bit, ma lui è su 8)
								MOV r4, #0		; r4 = result
								MOV r5, #0		; r5 = pos da confrontare
loop_concat						CMP r5, r1		; fine ciclo
								BGE end_concat
								
								LDRH r6, [r0]	; metto vet[i] in r6
								ADD r0, r0, #2	; vet++
								ADD r5, r5, #1	; pos++
								LSL r6, r6, #16	; sposto in parte alta
								
								CMP r5, r1			; se sono alla fine (dispari)
								MOVEQ r7, #0x0000
								LDRHNE r7, [r0]		; vet[i+1] in r7
								ADDNE r0, r0, #2	; vet++
								ADDNE r5, r5, #1	; pos++
								ORR r6, r6, r7		
								ADDS r4, r4, r6		; result+=
								MOVVS r8, #-1
								STRBVS r8, [r2]		; se overflow, alarm = -1
								B loop_concat
								
end_concat						MOV r0, r4		; torna result
								POP{r4-r8, r10-r11, pc}
								ENDP								
