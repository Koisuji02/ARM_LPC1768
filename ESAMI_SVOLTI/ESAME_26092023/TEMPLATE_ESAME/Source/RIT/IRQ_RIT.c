...
void svuotaVet(unsigned short int* vet, unsigned int index);
				...
				if((LPC_GPIO2->FIOPIN & (1<<11)) == 0){   //bottone premuto ma in modalità interrupt con debounce attivo
					...
					switch(debounce1){
						case 1: // dopo il time_interval con cui init_RIT (1 colpo, no debounce)
							...
							// AZIONE KEY1
							enable_timer(0); // inizio conteggio 300ms
					...
					disable_RIT();
					reset_RIT();
					disable_timer(0); // stoppo timer
					reset_timer(0);
					if(flagModify == 0){ // < 300ms
						var += 1;
					}
					flagModify = 0; // riazzero flag
