...
int disattivato =  0;
int turno = 0; // per lo switch di turno
...
void RIT_IRQHandler(void) {
				...
				if((LPC_GPIO2->FIOPIN & (1<<11)) == 0){   //bottone premuto ma in modalità interrupt con debounce attivo
					...
					switch(debounce1){
						case 1: // dopo il time_interval con cui init_RIT (1 colpo, no debounce)
							...
							// AZIONE KEY1
							if(turno == 2){ // se sono passato al turno da KEY2 (switch)
								vet[index] = var;
								index++;
								var = 0;
							}
							turno = 1;
							var++;
							enable_timer(0); // ad ogni pressione valore aggiornato lampeggio
				...
				if((LPC_GPIO2->FIOPIN & (1<<12)) == 0){   //bottone premuto ma in modalità interrupt con debounce attivo
					...
					switch(debounce2){
						case 1: // dopo il time_interval con cui init_RIT (1 colpo, no debounce)
							...
							// AZIONE KEY2
							if(turno == 1){ // se sono passato al turno da KEY2 (switch)
								vet[index] = var;
								index++;
								var = 0;
							}
		...
		if ((LPC_GPIO1->FIOPIN & (1 << 29)) == 0) { // JOYSTICK UP
				...
				enable_timer(1); // visualizzazione res[i]
				disattivato = 1; // nella visualizzazione impedisco altri valori
		}
