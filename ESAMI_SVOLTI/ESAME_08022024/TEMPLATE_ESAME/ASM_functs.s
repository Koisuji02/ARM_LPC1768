...
end_sort						MOV r0, r1                      ; r0 = metto val e torno
								...
								EXPORT svuotaVet ; cambio per 32 bit
...
contatore_inrange				PROC
								PUSH {r4-r8, r10-r11, lr}  		; Salva i registri utilizzati
								; r0 = vet
								; r1 = N = posizione massima (index)
								MOV r4, #0				; totale da tornare
								MOV r11, #4				; solo per la mul
								SUB r1, r1, #1			; r5 = offset totale per ultimo elemento
								MUL r5, r1, r11			
								ADD r1, r1, #1			; faccio tornare normale per confronto
								LDR r6, [r0]			; r6 = vet[0]
								ADD r5, r0, r5			; r5 = indirizzo di vet[N-1]
								LDR r7, [r5]			; r7 = vet[N-1]
								CMP r6, r7				; se r6 > r7, li switcho (così confronto giusto)
								MOVGT r8, r6
								MOVGT r6, r7
								MOVGT r7, r8
								MOV r8, #0				; r8 = #elementi da confrontare con N
								; così ho da confrontare vet[i] con r6<vet[i]<r7
loop_confronto_inrange			CMP r8, r1
								BGE exit_end
								LDR r10, [r0]			; r10 = vet[i]
								ADD r0, r0, #4			; vet++
								ADD r8, r8, #1			; #elementi++
								CMP r10, r6				; se r10 <= r6, ignora
								BLE loop_confronto_inrange
								CMP r10, r7				; se r10 >= r7, ignora
								BGE loop_confronto_inrange
								ADD r4, r4, #1
								B loop_confronto_inrange
