...
check_fibonacci					PROC
								MOV r12, sp			; r12 = sp per boundary
								PUSH {r4-r8, r10-r11, lr}  		; Salva i registri utilizzati
								; r0 = vet
								; r1 = pos
								; r2 = M = 14
								; r3 = val
								; sp(0) = BOUNDARY
								LDRB r4, [r12]		; r4 = boundary = 2
								MOV r5, #0			; pos da confrontare
								
								; primi 2 numeri di fib
								; per 0
								MOV r6, #0			; 0 di fib
								MOV r7, #1			; 1 di fib
								SUB r8, r3, r6		; val - 0
								CMP r8, #0			; se r8 < 0, devo fare r6 - r3 (val assoluto)
								SUBLT r8, r6, r3
								CMP r8, r4			; se r8 <= boundary, esco
								BLE end_check_fibonacci_not
								ADD r5, r5, #1
								
								; per 1
								SUB r8, r3, r7		; val - 1
								CMP r8, #0			; se r8 < 0, devo fare r7 - r3 (val assoluto)
								SUBLT r8, r7, r3
								CMP r8, r4			; se r8 <= boundary, esco
								BLE end_check_fibonacci_not
								ADD r5, r5, #1
								MOV r10, #0			; somma dei 2 numeri prima
								
loop_check_fibonacci			CMP r5, r2			; se finisco ciclo, tutti i confronti vanno bene
                                BGE end_check_fibonacci_ok
								ADD r10, r7, r6		; fib = fib[-1] + fib[-2]
								SUB r8, r3, r10		; val - fib
								CMP r8, #0			; se r8 < 0, devo fare r10 - r3 (val assoluto)
								SUBLT r8, r10, r3
								CMP r8, r4			; se r8 <= boundary, esco
								BLE end_check_fibonacci_not
								; altrimenti
								MOV r6, r7			; fib[-2] = fib[-1]
								MOV r7, r10			; fib[-1] = fib
								ADD r5, r5, #1		; pos++
								B loop_check_fibonacci
								
								
end_check_fibonacci_ok			ADD r0, r0, r1		; vet[index] = 1^ pos libera (dim di vet = 1)
								STRB r3, [r0]		; metto val in vet[index]
								MOV r0, #1			; torno 1
								POP{r4-r8, r10-r11, pc}
								
end_check_fibonacci_not			MOV r0, #0			; torno 0
								POP{r4-r8, r10-r11, pc}
								ENDP
