...
end_svuotaU						POP{r4-r8, r10-r11, pc}
								ENDP
...
compress						PROC
								...
								; r1 = index
								; r2 = res[]
								MOV r4, #0			; totale da mettere in r0 (result)
								MOV r5, #0			; posizione da confrontare con index per non sforare
								SUB r1, r1, #1		; tolgo 1 all'index in modo che mi fermo a N-1
								
loop_compress					CMP r5, r1
								BGE end_compress
								LDRB r6, [r0]		; r6 = vet[i]
								ADD r0, r0, #1
								LDRB r7, [r0]		; r7 = vet[i+1]
								SUB r8, r6, r7		; r8 = vet[i]-vet[i+1]
								STRB r8, [r2]		; res[i] = r8
								ADD r5, r5, #1		; posizione++
								ADD r2, r2, #1		; res++
								CMP r8, #0
								BGE loop_compress	; solo se r8 < 0, faccio r4++
								ADD r4, r4, #1
								B loop_compress
								
end_compress					MOV r0, r4			; torno result
								POP{r4-r8, r10-r11, pc}
								ENDP
