...
loop_compare					CMP r5, r2		; se fine esco
								BGE end_compare
								
								LDRB r6, [r1]	; vet2[i]
								ADD r1, r1, #1	; vet2++
								CMP r6, #0		; se vet2[i] != 0, vuol dire che posizione era valida, visto il mio programma
								ADDEQ r0, r0, #1	; se vet2[i] = 0, posizione illegale, skippo entrambi e vado al prossimo elemento
								ADDEQ r5, r5, #1	; pos++
								BEQ loop_compare
								...
								ADD r5, r5, #1		; pos++
								CMP r6, r7
								ADDNE r4, r4, #1	; se vet[i] != vet2[i], r4++
								B loop_compare
								
end_compare						MOV r0, r4
								POP{r4-r8, r10-r11, pc}
								ENDP
