...
unsigned char vet[N] = {1,1,1,2,0,2,1,3,2};
unsigned char vet2[N];
unsigned int result = 0;
unsigned int index = 0; // uso index per entrambi, visto che la traccia non mi dice di non farlo
int scaler = 4;
//int value_tmp = 0;
...
int main (void) {
  ...
  BUTTON_init();												/* BUTTON Initialization              */
	...
	init_timer(0,0,0,3,0x2FAF08/scaler); // 0.125s = lampeggio da 0.25s
	
	init_timer(1,0,0,3,0x2FAF08/scaler); // 0.125s = lampeggio da 0.25s
	
	init_timer(2,0,0,3,0x17D784/scaler); // 0.125s/2 = 0.0625 (lampeggio da 0.125s)
	
	//init_timer(3,0,0,3,0x23C3460); // 1.5s
	
	svuotaVet(vet2, N); // inizializzo vuoto (anche se di default è vuoto, nel dubbio)
