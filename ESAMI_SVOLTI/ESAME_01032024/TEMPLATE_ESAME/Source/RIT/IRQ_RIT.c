...
uint8_t extractBits(int value);
unsigned char check_fibonacci(unsigned char* VETT, unsigned char dim, unsigned char elem, unsigned char VAL, unsigned char BOUNDARY);
...
int disattivato =  0;
int pressioneKey2 = 0; // per rilevare prima o seconda pressione
				...
				if((LPC_GPIO2->FIOPIN & (1<<11)) == 0){   //bottone premuto ma in modalità interrupt con debounce attivo
					...
					switch(debounce1){
						case 1: // dopo il time_interval con cui init_RIT (1 colpo, no debounce)
							...
							if(index == N){ // se ho riempito vettore
								reset_timer(3);
								enable_timer(3);
							...
							if(pressioneKey2 == 0){ // 1^ pressione = attivo timer
								pressioneKey2 = 1;
								reset_timer(2);
								enable_timer(2); // prima pressione
								LED_Out(0); // spegne dopo NON ACCETTABILE
								
							} else{ // 2^ pressione = rilevo TC
								pressioneKey2 = 0;
								...
								val = extractBits(value_tmp);
								if(index == 0){
									if((val%2) == 0){ // 1° elemento deve essere pari
										risultato = check_fibonacci(vett,N,M,val,2);  // boundary = 2 (non specificato nel testo)
										if(risultato == 1){ // ACCETTABILE
											index++;
											enable_timer(0); // controlla LED lampeggio
											disattivato = 1;
											enable_timer(1); // controlla spegnimento timer0
										} else{ // NON ACCETTABILE
											LED_Out(0xFF);
											counterNonAccettabili++;
										}
									}
								} else{ // non 1° elemento di vet
									if(((vett[index]%2) == 0 && (val%2) != 0) || ((vett[index]%2) != 0 && (val%2) == 0)){ // se opposto ad elemento prima
										risultato = check_fibonacci(vett,N,M,val,2);  // boundary = 2 (non specificato nel testo)
										if(risultato == 1){ // ACCETTABILE
											index++;
											enable_timer(0); // controlla LED lampeggio
