...
sort_decrescente_exit			POP{r4-r8, r10-r11, pc}
								...
								EXPORT extractBits		; bit [10,3] di TC
			
extractBits						PROC
								PUSH {r4-r8, r10-r11, lr}  		; Salva i registri utilizzati
								; r0 = value_tmp
								LSR r0, r0, #3
								AND r0, r0, #0xFF		; prendo gli 8 bit shiftati (10-3) del value e lo metto in val
								
								POP{r4-r8, r10-r11, pc}
								ENDP
								
								EXPORT check_fibonacci
								; index non è richiesto come parametro, quindi inserisco alla 1^ posizione dove vet[i] == 0
check_fibonacci					PROC
								; qua commento, perchè faccio il push dopo per lo stack
								;PUSH {r4-r8, r10-r11, lr}  		; Salva i registri utilizzati
								; r0 = vet
								; r1 = N (# vet)
								; r2 = M (# elementi fibonacci da calcolare e confrontare con boundary rispetto a val)
								; r3 = val
								; sp = BOUNDARY (distanza da confrontare tra val e elemento di fibonacci)
								MOV r12, sp
								PUSH {r4-r8, r10-r11, lr}  		; Salva i registri utilizzati
								
								LDR r4, [r12]		; r4 = BOUNDARY
								MOV r5, #0			; posizione vet confronto
								MOV r7, #0			; registro per 0 fibonacci
								MOV r8, #1			; registro per 1 fibonacci
								
loop_posizione_vet_libera		CMP r5, r1
								BGE no_liberi		; non ho elementi liberi se posizione >= N, quindi esco
								LDRB r6, [r0]		; r6 = vet[i] (Byte-> uint8_t)
								CMP r6, #0
								; se vet[i] = 0, vuoto, quindi trovo posizione libera a vet[i] = r0
								; altrimenti vado avanti nel loop (istruzione condizionale)
								ADDNE r5, r5, #1
								ADDNE r0, r0, #1	; Byte (+1)
								BNE loop_posizione_vet_libera
								; r0 = 1^ posizione libera del vettore, dove scriverò val
								
								; calcolo boundary per 0 e 1, poi entro nel ciclo di fibonacci
								; 0
								SUB r10, r3, r7			; r10 = distanza val-0 >? boundary
								CMP r10, #0				; se r10 < 0, faccio 0 - r3 e non il contrario
								SUBLT r10, r7, r3		; (altrimenti vado avanti con r3-r7)
								CMP r10, r4				; se distanza <= boundary, esco (come no_liberi)
								BLE no_liberi
								
								;1
								SUB r10, r3, r8			; r10 = distanza val-0 >? boundary
								CMP r10, #0				; se r10 < 0, faccio 0 - val e non il contrario
								SUBLT r10, r8, r3		; (altrimenti vado avanti con r3-r7)
								CMP r10, r4				; se distanza <= boundary, esco (come no_liberi)
								BLE no_liberi
								
								MOV r11, #0				; contatore ciclo fibonacci
								SUB r2, r2, #2			; non conto lo 0 e l'1 (fuori dal loop)
								
loop_fibonacci					; ora vado avanti fino a M
								CMP r11, r2
								BEQ valido
								ADD r5, r7, r8			; r5 = riuso per somma fibonacci
								MOV r7, r8				; r7 = vecchio r8 (numero maggiore di fibonacci)
								MOV r8, r5				; r8 = nuovo maggiore (somma)
								
								SUB r10, r3, r8			; r10 = distanza val-fibonacci_maggiore_nuovo
								CMP r10, #0				; se r10 < 0, faccio fibonacci - val e non il contrario
								SUBLT r10, r8, r3		; (altrimenti vado avanti con r3-r7)
								CMP r10, r4				; se distanza <= boundary, esco (come no_liberi)
								BLE no_liberi
								ADD r11, r11, #1		; counter_loop_fibonacci++
								B loop_fibonacci
								
valido							STRB r3, [r0]			; scrivo val in vet[i]
								MOV r0, #1				; torno 1
								POP{r4-r8, r10-r11, pc}
								
no_liberi						MOV r0, #0				; non ok -> torno 0
								POP{r4-r8, r10-r11, pc}
								ENDP
