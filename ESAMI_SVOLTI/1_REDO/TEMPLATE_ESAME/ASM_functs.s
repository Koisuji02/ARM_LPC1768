...
end_sort						MOV r0, r1                      ; r0 = metto val e torno
								...
								EXPORT svuotaVet ; modificata per 32 bit
...
loop_negative_and_odd			CMP r5, r1		; fine ciclo
								BGE end_negative_and_odd
								LDR r6, [r0]	; vet[i]
								ADD r0, r0, #4	; vet++
								ADD r5, r5, #1	; pos++
								; DISPARI
								AND r7, r6, #1
								CMP r7, #0 ; se pari skip
								BEQ loop_negative_and_odd
								; NEGATIVO
								CMP r6, #0	; se positivo skip
								BGE loop_negative_and_odd
								; se dispari e negativo aggiungi
								ADDLT r4, r4, #1
								B loop_negative_and_odd
								
								
end_negative_and_odd			MOV r0, r4
								POP{r4-r8, r10-r11, pc}
								ENDP
