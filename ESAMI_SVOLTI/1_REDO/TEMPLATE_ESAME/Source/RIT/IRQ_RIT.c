...
void RIT_IRQHandler(void) {
		...
		if ((LPC_GPIO1->FIOPIN & (1 << 29)) == 0) { // JOYSTICK UP
				...
				// AZIONE DOWN
				var1 -= 8;
		} else if ((LPC_GPIO1->FIOPIN & (1 << 25)) == 0) { // JOYSTICK SELECT
				// AZIONE SELECT
				//ready = 1;
				vet[index] = var1;
				index++;
				...
				if(index == N){ // vettore pieno
					result = count_negative_and_odd(vet, N);
					svuotaVet(vet, N);
					index = 0;
					if(result == 0){
						LED_On(7);				// LED4 = 7 (schematico)
						enable_timer(0); // attiva per blink
					} else{
						LED_Off(7);
						LED_OutCustom(result);
					}
				}
		}
