...
end_sort						MOV r0, r1                      ; r0 = metto val e torno
								...
								EXPORT svuotaVet ; passare su 32 bit (unsigned int)
...
avg_vett						PROC
								...
								; r0 = vet
								; r1 = N
								; r2 = indirizzo di flag (32 bit, da cui scrivo in byte = 8 bit char)
								MOV r4, r0	; per ripristinare dopo media
								;MOV r5, #0	; posizione da cmp (riazzera al 2° ciclo)
								MOV r6, #0	; media (prima totale, poi media)
								MOV r7, #0	; # elementi (per la media) e uso anche per confronto ciclo
								
ciclo_avg						CMP r7, r1	; se finisco, vado al ciclo del confronto
								BGE pre_ciclo_confronto_avg
								LDR r8, [r4]	; vet[i]
								ADD r4, r4, #4	; vet++
								ADD r6, r6, r8	; totale += vet[i]
								ADD r7, r7, #1	; contatore elementi
								B ciclo_avg
								
pre_ciclo_confronto_avg			MOV r4, r0		; riazzero a vet
								SDIV r6, r6, r7	; r6 = avg
								MOV r7, #0		; r7 = contatore elementi da tornare sopra la media
								MOV r8, #0		; #elementi confronto fine ciclo
ciclo_confronto_avg				CMP r8, r1		; se finito vettore
								BGE controllo_finale
								LDR r5, [r4]	; vet[i]
								CMP r5, r6		; se r5 > r6, ovvero vet[i] > media
								ADDGT r7, r7, #1
								ADD r4, r4, #4	; vet++
								ADD r8, r8, #1	; #elementi++
								B ciclo_confronto_avg
								
controllo_finale				MOV r0, r7		; torno totale elementi sopra media
								TST r7, #1		; faccio and e controllo se pari
								; EQ = PARI (and da 0)
								; NE = DISPARI (and da 1)
								MOV r4, #1
								STRBEQ r4, [r2] ; flag = 1, altrimenti già a 0
								POP{r4-r8, r10-r11, pc}
								ENDP
