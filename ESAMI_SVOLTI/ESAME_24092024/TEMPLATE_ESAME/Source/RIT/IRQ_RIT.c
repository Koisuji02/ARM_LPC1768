...
void svuotaVet(unsigned int* vet, unsigned int index);
				...
				if((LPC_GPIO2->FIOPIN & (1<<10)) == 0){   //bottone premuto ma in modalità interrupt con debounce attivo
					...
					switch(debounce0){
						case 1: // dopo il time_interval con cui init_RIT (1 colpo, no debounce)
							...
							else if(var != vet[index-1]){
								vet[index] = var;
							}
							index++;
							if(index == N){ // vettore pieno
								disable_timer(0); // fino alla prossima esecuzione
								LED_Out(0); 
								result = avg_vett(vet, N, &flag);
								svuotaVet(vet, N);
								index = 0;
								if(flag == 0){
									shiftTmp = !(result << 24);
