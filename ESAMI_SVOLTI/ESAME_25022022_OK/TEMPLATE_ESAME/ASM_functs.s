...
traduzione_morse PROC
				PUSH {r4-r8, r10-r11, lr}
				
				; r0 = indirizzo vettore
				; r1 = indirizzo vettore out
				MOV r4, #0					; counter parole
				MOV r6, #0				; parola attuale
				
loop_morse		LDRB r5, [r0]				; vet[i]
				SUBS r5, r5, #1				; se vet[i] - 1 > 0, counter++
				ADD r5, r5, #1				; faccio tornare al valore di prima, prima del SUBS per l'if
				BLE gestione_loop_morse
				; gestione valore shiftato da mettere dentro a vet
				LSR r6, r6, #4
				STRB r6, [r1]
				; dovrei convertirlo, ma troppo lungo; per convertirlo confronto quale sia lettera e salvo l'offset rispetto ad A, così alla stampa tiro fuori A+offset per ogni lettera
				MOV r6, #0
				ADDGT r4, r4, #1
				CMPGT r5, #4					; se vet[i] == 4 ho finito
				BEQ end_loop_morse
				ADD r0, r0, #1				; prossimo elemento (8 bit = 1 Byte)
				B loop_morse
