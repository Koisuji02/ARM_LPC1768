...
end_sort						MOV r0, r1                      ; r0 = metto val e torno
								...
								EXPORT svuotaVet ; cambio a 32bit
...
contatore_inrange				PROC
								...
								; r0 = vet
								; r1 = N
								MOV r4, #0			; r4 = totale da tornare
								MOV r5, #0			; r5 = pos da confrontare con N
								LDR r6, [r0]		; r6 = vet[0]
								MOV r7, #4			; dim vet[i]
								MUL r7, r1, r7		; r7 = offset
								ADD r7, r0, r7		; r7 = indirizzo ultimo elemento +4
								SUB r7, r7, #4		; r7 = vero indirizzo vet[N-1]
								LDR r8, [r7]		; r8 = vet[N-1]
								
loop_contatore_inrange			CMP r5, r1			; se finito vettore
								BGE end_contatore_inrange
								LDR r7, [r0]		; r7 = vet[i]
								ADD r0, r0, #4		; vet++
								ADD r5, r5, #1		; # elementi++
								CMP r7, r6			; se vet[i] <= vet[0] skip
								BLE loop_contatore_inrange
								CMP r7, r8			; se vet[i] >= vet[N-1] skip
								BGE loop_contatore_inrange
								ADD r4, r4, #1		; totale++
								B loop_contatore_inrange
								
end_contatore_inrange			MOV r0, r4			; torno totale
								POP{r4-r8, r10-r11, pc}
								ENDP
