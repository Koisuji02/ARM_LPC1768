...
count_negative_and_odd			PROC
								...
								MOV r5, #0				; confronto index
loop_count						CMP r5, r1
								BGE end_count
								LDR r6, [r0]			; vet[i]
								ADD r0, r0, #4			; i++
								ADD r5, r5, #1			; # elementi ++
								CMP r6, #0				; vet[i] < 0 -> negativo
								BGE loop_count
								TST r6, #1				; faccio l'and con 1 -> se 0 pari, BEQ
								BEQ loop_count
								ADD r4, r4, #1			; se dispari e negativo
								B loop_count
								
end_count						MOV r0, r4				; torno totale
								POP{r4-r8, r10-r11, pc}
								ENDP
